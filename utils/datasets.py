...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            if hyp['time_seq']>0:
                img, labels = load_mosaic_temporal(self, index)
            else:
...
def load_mosaic_temporal(self, index):
    # YOLOv5 4-mosaic loader. Loads 1 image + 3 random images into a 4-image mosaic
    labels4, segments4 = [], []
    s = self.img_size
    self.temporal_count = int(self.temporal_count % self.hyp['time_seq'])
    if self.temporal_count == 0:
        yc, xc = (int(random.uniform(-x, 2 * s + x)) for x in self.mosaic_border)  # mosaic center x, y
        indices = [index] + random.choices(self.indices, k=3)  # 3 additional image indices
        random.shuffle(indices)
        self.yc_temp, self.xc_temp = yc, xc
        self.indices_temp = indices
    else:
